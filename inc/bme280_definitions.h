 ...
 */
typedef void (*bme280_delayms)(uint32_t delay_time);
///@}
...
struct spi_bus_data {
	...
	hal_spi_dev_struct *spi_handle;	// handle to spi bus
	uint32_t NCS_gpio; // chip select gpio port
	uint32_t NCS_pin;		// chip select pin nr.
};
